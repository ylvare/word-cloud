...
function generate() {
  layout
    ...
    // Get fill color palette from the select input
    (fill = d3.scale[d3.select("#color-palette").property("value")]().domain([
      0, 1e6,
    ])),
    (fontSize = d3.scale[
...
!(function (t) {
  ...
  function l(t, e, n) {
    if (!t.sprite) {
      ...
      for (
        var x = w.getImageData(0, 0, (g << 5) / m, v / m).data, M = [];
        --n >= 0;

      ) {
        t = e[n];
        for (
          var l = t.width, b = l >> 5, u = t.y1 - t.y0, z = t.padding, k = 0;
          u * b > k;
          k++
        )
          M[k] = 0;
  ...
  if ("undefined" != typeof document)
    (p = document.createElement("canvas")),
      ...
      (p.height = 1),
      (m = Math.sqrt(
        p.getContext("2d").getImageData(0, 0, 1, 1).data.length >> 2
      )),
      (p.width = (g << 5) / m),
      (p.height = v / m);
  else {
...
form.selectAll("input[type=number]").on("click.refresh", function () {
  this.value !== this.defaultValue &&
    (generate(), (this.defaultValue = this.value));
}),
...
var stopWords =
  ...
  discard = /^(@|https?:|\/\/)/,
  htmlTags =
    /(<[^>]*?>|<script.*?<\/script>|<style.*?<\/style>|<head.*?><\/head>)/g,
...
d3.select("#random-palette").on("click", function () {
  ...
  (function () {
    ...
    d3
      .selectAll("#angle-count, #angle-from, #angle-to")
      .on("change", t)
      .on("mouseup", t),
      t(),
